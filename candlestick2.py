...
def fetch_prices(ticker, timeframe, start_str, end_str):
    ...
    # Normalize columns (also keep Volume if present)
...
# ---------- Tools ----------
def dist_point_to_segment_pixels(ax, x0, y0, x1, y1, xp, yp):  # (dup guard)
    trans = ax.transData.transform
    (X0, Y0) = trans((x0, y0)); (X1, Y1) = trans((x1, y1)); (XP, YP) = trans((xp, yp))
    vx, vy = X1 - X0, Y1 - Y0
    wx, wy = XP - X0, YP - Y0
...
def dist_point_to_point_pixels(ax, xa, ya, xb, yb):  # (dup guard)
    trans = ax.transData.transform
    (XA, YA) = trans((xa, ya)); (XB, YB) = trans((xb, yb))
    return np.hypot(XA - XB, YA - YB)
...
# ---------- Chart drawing ----------
def _compute_bimonthly_xticks(index_dt):
    """return positions (int indices) and labels every 2 months (YY-MM)"""
    if len(index_dt) == 0:
        return [], []
    # pick first index of each month
    df_idx = pd.DataFrame({"dt": index_dt})
    df_idx["ym"] = df_idx["dt"].dt.to_period("M")
    first_pos = df_idx.groupby("ym").head(1).index.to_numpy()
    # take every 2nd month starting from the first
    bi_pos = first_pos[::2]
    labels = [index_dt[i].strftime("%y-%m") for i in bi_pos if i < len(index_dt)]
    bi_pos = [i for i in bi_pos if i < len(index_dt)]
    return bi_pos, labels

def draw_candles(ax_price, ax_vol, df, log_scale=False, style="candle", bar_color="black",
    ...
    if draw_n > 0:
        ...
        vol = slice_df["Volume"].to_numpy() if "Volume" in slice_df.columns else np.zeros(draw_n)

        up = (c >= o); down = ~up

        if style == "candle":
            ax_price.vlines(xd[up], l[up], h[up], colors="green")
            ax_price.vlines(xd[down], l[down], h[down], colors="red")
            ax_price.bar(xd[up], (c[up]-o[up]), bottom=o[up], width=0.6, align="center", color="green")
            ax_price.bar(xd[down], (c[down]-o[down]), bottom=o[down], width=0.6, align="center", color="red")
            # Volume colors follow candle color
            vcolors = np.where(up, "green", "red")
        else:
            for i in range(draw_n):
                xi = xd[i]
                ax_price.vlines(xi, l[i], h[i], colors=bar_color)
                tw = 0.25
                ax_price.hlines(o[i], xi - tw, xi, colors=bar_color)
                ax_price.hlines(c[i], xi, xi + tw, colors=bar_color)
            vcolors = np.array([bar_color]*draw_n)

        # --- Volume panel ---
        if "Volume" in slice_df.columns:
            ax_vol.bar(xd, vol, width=0.6, align="center", color=vcolors, alpha=0.6)

    # X ticks (bimonthly: every two months)
    if n:
        idx_dt = pd.to_datetime(df.index)
        xticks, labels = _compute_bimonthly_xticks(idx_dt)
        ax_vol.set_xticks(xticks); ax_vol.set_xticklabels(labels, rotation=0, ha="center")
    else:
        ax_vol.set_xticks([]); ax_vol.set_xticklabels([])

    ax_price.set_xlabel(""); ax_price.set_ylabel("Price"); ax_price.grid(True, linestyle=":")
    ax_vol.set_ylabel("Vol"); ax_vol.grid(True, linestyle=":")

    try: ax_price.set_yscale("log" if log_scale else "linear")
    except Exception: ax_price.set_yscale("linear")

    # X limits so future area stays blank
    if x_full_len is not None:
        ax_price.set_xlim(0, max(0, x_full_len-1))
        ax_vol.set_xlim(0, max(0, x_full_len-1))
    else:
        ax_price.set_xlim(0, max(0, n-1))
        ax_vol.set_xlim(0, max(0, n-1))

    # sharex already; keep tight layout
    ax_vol.set_xlabel("")
...
class TVStyleApp:
    ...
    LINEWIDTHS = ["1","2","3","4"]
    SPEEDS = {"Slow": 100, "Medium": 50, "Fast": 5}  # ms per bar
    ...
    def __init__(self, root):
        ...
        # Figure with 2 rows (price + volume)
        self.fig = plt.Figure(figsize=(12,7), dpi=100)
        gs = self.fig.add_gridspec(nrows=2, ncols=1, height_ratios=[3,1], hspace=0.08)
        self.ax_price = self.fig.add_subplot(gs[0, 0])
        self.ax_vol   = self.fig.add_subplot(gs[1, 0], sharex=self.ax_price)
        self.canvas = FigureCanvasTkAgg(self.fig, master=root)
        self.canvas_widget = self.canvas.get_tk_widget()
        self.canvas_widget.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        self.toolbar = NavigationToolbar2Tk(self.canvas, root); self.toolbar.update()

        # Tools (attach to price axis)
        self.snap_x = None
        self.line_tool = DrawEditLineTool(self.ax_price, snap_x=lambda x: None,
                                          get_current_color_cb=self.get_current_color,
                                          get_current_lw_cb=self.get_current_lw)
        self.pen_tool  = PenTool(self.ax_price, get_current_color_cb=self.get_current_color)

        self.canvas_widget.bind("<Button-3>", self.on_right_click_canvas)
        self.root.bind("<Delete>", self.on_delete_key)

        # Menu variables (before menus)
        ...
        self.last_temp_dt = None  # keep TEMP for reset
    ...
    def _build_menus(self):
        ...
        rmenu.add_separator()
        rmenu.add_command(label="Reset Replay", command=self.reset_replay)
    ...
    def on_right_click_canvas(self, event):
        inv = self.ax_price.transData.inverted()
    ...
    def _redraw_preserving(self, df_past_only, full_len):
        ...
        self.snap_x = draw_candles(
            self.ax_price, self.ax_vol, df_past_only, log_scale=self.log_var.get(), style=style,
    ...
    def apply_refresh(self):
        """Refresh data & redraw chart. Fixes tz-aware vs tz-naive comparison errors."""
        ...
        if not ticker:
            messagebox.showwarning("Input", "Please enter a ticker.")
            return
        ...
        # normalize tz: make index tz-naive
        ...
        if isinstance(temp_dt, pd.Timestamp) and temp_dt.tzinfo is not None:
            try:
                temp_dt = temp_dt.tz_localize(None)
            except Exception:
                temp_dt = pd.Timestamp(temp_dt.to_pydatetime().replace(tzinfo=None))
        self.last_temp_dt = temp_dt  # keep for reset
    ...
    def reset_replay(self):
        """Return to TEMP point without refetching; keep drawings."""
        if self.full_df is None:
            return
        if self.last_temp_dt is None:
            # no temp -> all past
            self.current_df = self.full_df.copy()
            self.future_df  = self.full_df.iloc[0:0].copy()
        else:
            mask = self.full_df.index <= self.last_temp_dt
            if mask.sum() == 0 or mask.sum() == len(self.full_df):
                self.current_df = self.full_df.copy()
                self.future_df  = self.full_df.iloc[0:0].copy()
            else:
                self.current_df = self.full_df.loc[mask].copy()
                self.future_df  = self.full_df.loc[~mask].copy()
        self.replay_index = 0
        self.replay_state = "stopped"
        self.replay_btn.config(text="Replay ▶")
        self._redraw_preserving(self.current_df, len(self.full_df))
